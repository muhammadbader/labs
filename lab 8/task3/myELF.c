...
void examineELF_File(){
    ...
    if(Currentfd != -1)
        close(Currentfd);
    Currentfd = open(filename,O_RDONLY); /*O_RDWR in short 2, 0677 permission to read*/
    if(Currentfd == -1){
   ...
   */
    if((entry_map = mmap(0,examinee.st_size ,PROT_READ, MAP_SHARED | MAP_32BIT, Currentfd, 0)) == MAP_FAILED){
        close(Currentfd);
        Currentfd = -1;
        perror("mmap failed\n");
        _exit(EXIT_FAILURE);
...
void relocationtables()
{
    ...
    int check = 0;
    Elf32_Ehdr* hd = (Elf32_Ehdr *) entry_map;
    unsigned int nSections = (unsigned int)hd->e_shnum;
    unsigned int absoff = (unsigned int) hd->e_shoff;
    unsigned int indx = (unsigned int) hd->e_shstrndx;
    Elf32_Shdr *sd = (Elf32_Shdr *) ((void*) hd + absoff) ,sr ,ss ;
    Elf32_Shdr * tmp = sd;
    char * strtab;
    Elf32_Rel* rel;
    for(int i=0; i<nSections; i++){
        if(SHT_DYNSYM == sd[i].sh_type){
            ss = sd[i];
            strtab = entry_map +(tmp[ss.sh_link]).sh_offset;
            Elf32_Sym * sym = (Elf32_Sym *) (entry_map + ss.sh_offset);
            for(int i=0; i<nSections; i++){
                if(SHT_REL == sd[i].sh_type){
                    unsigned int nameoff = sd[i].sh_name;
                Elf32_Shdr* sth = &sd[indx];
                char* name = entry_map + sth-> sh_offset;
                    printf("%s\n",name+nameoff);
                    sr = sd[i];
                    rel = (Elf32_Rel*)(entry_map + sr.sh_offset);
                    check = 1;
                    for(int j=0;j< (int)sr.sh_size / (int)sr.sh_entsize; j++){
                        unsigned int info = (int)rel[j].r_info;
                        unsigned int off = (int)rel[j].r_offset;
                        char* name = strtab + sym[ELF32_R_SYM(info)].st_name;
                        unsigned int value = sym[ELF32_R_SYM(info)].st_value;
                        printf("%08x %08x %d %d %s:\n",info,off,ELF32_R_TYPE(info),value,name);
                    }
                }
            }
        }
